...
num_iterations = 1
modes = ['eng_mfcc_env','adib_bark','ae_bark','unsupervised_bark','RI_bark','KSH_bark','RI_KSH_bark']
...
average_precisions = np.zeros((len(modes),num_iterations))
for md in range(len(modes)):
    for it in range(num_iterations):
        average_precisions[md,it] = np.mean(np.reciprocal(rankings[md,it]+1))
    print('Average Precision ' + modes[md] + ': ' + str(np.round(np.mean(average_precisions[md]),3)) + ' +- ' + str(np.round(np.std(average_precisions[md]),3)))
