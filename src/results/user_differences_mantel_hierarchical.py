...
embeddings_ref = np.load('data/processed/energy_bands/embeddings_ref_energy_bands.npy')
embeddings_imi_pre = np.load('data/processed/energy_bands/embeddings_imi_energy_bands.npy')
...
distance_matrices_ref = np.zeros((num_features,18,18))
distance_matrices_imi = np.zeros((14,num_features,18,18))

for i in range(14):
    for emb in range(latent_dim):
        for j in range(18):
            for k in range(18):
                distance_matrices_ref[emb,j,k] = euclidean(embeddings_ref[j,emb], embeddings_ref[k,emb])
                distance_matrices_imi[i,emb,j,k] = euclidean(embeddings_imi[i,j,emb], embeddings_imi[i,k,emb])
