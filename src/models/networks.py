...
class VAE_Interim(tf.keras.Model):
    ...
    def __init__(self, latent_dim, num_mels):
        super(VAE_Interim, self).__init__()
        self.latent_dim = latent_dim
        self.encoder = tf.keras.Sequential(
            [
                tf.keras.layers.InputLayer(input_shape=(num_mels, 128, 1)),
...
class CVAE_Interim(tf.keras.Model):
    ...
    def __init__(self, latent_dim, num_mels, num_labels):
...
class CNN_Interim(tf.keras.Model):
    ...
    def __init__(self, num_mels, num_labels):
        ...
        self.encoder = tf.keras.Sequential(
            [
                ...
                tf.keras.layers.MaxPool2D(pool_size=2, padding='valid'),
                tf.keras.layers.Conv2D(filters=128, kernel_size=3, strides=1, activation='relu', padding='same'),
                tf.keras.layers.MaxPool2D(pool_size=2, padding='valid'),
                tf.keras.layers.Flatten(),
...
class CNN_FConv_Interim(tf.keras.Model):

    def __init__(self, num_labels, num_mels):
        super(CNN_FConv_Interim, self).__init__()
        self.encoder = tf.keras.Sequential(
            [
                tf.keras.layers.InputLayer(input_shape=(num_mels, 128, 1)),
