...
for m in range(len(modes)):
    ...
    if mode=='unsupervised' or mode=='unsupervised_bark' or mode=='ae_77':
    ...
    for it in range(3):
        ...
        if mode=='unsupervised' or mode=='unsupervised_bark' or mode=='ae_77':
            ...
            dec = layers.BatchNormalization()(dec)
            dec = layers.ReLU()(dec)
            dec = layers.Conv2DTranspose(filters=1, kernel_size=7, strides=2, padding='same', activation=None)(dec)
            dec = layers.BatchNormalization()(dec)
            dec = layers.ReLU()(dec)
            dec = layers.Conv2DTranspose(filters=1, kernel_size=7, strides=1, padding='same', activation=None)(dec)
            dec = layers.BatchNormalization()(dec)
            dec = layers.ReLU()(dec)
            ...
            checkpoint_path = "checkpoints_ae_77/cp_" + mode + "_" + str(it) + ".ckpt"
