...
for m in range(len(modes)):
    ...
    for it in range(5):
        ...
        x_cond = Concatenate(axis=-1)([x, encoder_class])
        z = Dense(latent_dim, activation='linear')(x_cond)

        encoder = Model([encoder_input, encoder_class], [z, encoder_class])

        # Decoder

        latent_inputs = Input(shape=(latent_dim,))
        decoder_classes = Input(shape=(8,))
        ...
        dec = layers.Conv2D(filters=32, kernel_size=9, strides=1, padding='same', activation=None)(dec)
        dec = layers.BatchNormalization()(dec)
        dec = layers.ReLU()(dec)
        dec = layers.UpSampling2D(size=(4, 2))(dec)
        dec = layers.Conv2D(filters=16, kernel_size=9, strides=1, padding='same', activation=None)(dec)
        dec = layers.BatchNormalization()(dec)
        dec = layers.ReLU()(dec)
        dec = layers.UpSampling2D(size=(4, 2))(dec)
        dec = layers.Conv2D(filters=8, kernel_size=9, strides=1, padding='same', activation=None)(dec)
        dec = layers.BatchNormalization()(dec)
        dec = layers.ReLU()(dec)
        ...
        decoder_output = decoder(encoder([[encoder_input, encoder_class]]))
        ...
        checkpoint_path = "checkpoints_adib_bark/iywdyulhcp_" + mode + "_" + str(it) + ".ckpt"
        ...
        embeddings_ref, _ = encoder([Pretrain_Dataset_Eval_REF_Expanded,Pretrain_Classes_Eval_REF_OneHot])
        embeddings_imi, _ = encoder([Pretrain_Dataset_Eval_IMI_Expanded,Pretrain_Classes_Eval_IMI_OneHot])
