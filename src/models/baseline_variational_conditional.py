...
for m in range(len(modes)):
    ...
    for cn in range(len(conds)):
        ...
        if cond=='ri':
            replacer_0 = replacements_0.get
            replacer_1 = replacements_1.get
        elif cond=='ksh':
            replacer_0 = replacements_03.get
            replacer_1 = replacements_03.get
        elif cond=='ri_ksh':
            replacer_0 = replacements_03.get
            replacer_1 = replacements_47.get

        Pretrain_Classes_REF_Num = np.array([replacer_0(n,n) for n in Pretrain_Classes_REF.astype('U13')])
        Pretrain_Classes_IMI_Num = np.array([replacer_1(n,n) for n in Pretrain_Classes_IMI.astype('U13')])
        Pretrain_Classes_Eval_REF = np.array([replacer_0(n,n) for n in Pretrain_Classes_Eval_REF.astype('U13')])
        Pretrain_Classes_Eval_IMI = np.array([replacer_1(n,n) for n in Pretrain_Classes_Eval_IMI.astype('U13')])

        Pretrain_Classes = np.concatenate((Pretrain_Classes_REF_Num,Pretrain_Classes_IMI_Num))

        num_classes = np.max(Pretrain_Classes)+1
        ...
        for it in range(num_iterations):
            ...
            model = adib_best_var_cond(latent_dim, num_classes)

            training_generator = tf.data.Dataset.from_tensor_slices(({"input": pretrain_dataset_train, "class": pretrain_classes_train}))
            training_generator = training_generator.batch(batch_size, drop_remainder=True)

            validation_generator= tf.data.Dataset.from_tensor_slices(({"input": pretrain_dataset_test, "class": pretrain_classes_test}))
            validation_generator = validation_generator.batch(batch_size, drop_remainder=True)

            with tf.device(gpu_name):
                history = model.fit(training_generator, validation_data=(validation_generator, None), epochs=epochs, callbacks=[EarlyStoppingAtMinLoss(7,0),LearningRateSchedulerCustom(4)], shuffle=True)
            ...
            for submodel in model.layers[2:3]:
                last_layer = submodel.get_layer("z_mean")
                last_output = last_layer.output
                input_layer = submodel.input
                extractor = keras.Model(input_layer, last_output)
            ...
            embeddings_ref = extractor.predict(([Pretrain_Dataset_Eval_REF_Expanded,Pretrain_Classes_Eval_REF_OneHot]))
            embeddings_imi = extractor.predict(([Pretrain_Dataset_Eval_IMI_Expanded,Pretrain_Classes_Eval_IMI_OneHot]))
