...
for m in range(len(modes)):
    ...
    if 'unsupervised_bark' in mode or 'ae' in mode:
    ...
    for it in range(5):
        ...
        if mode=='unsupervised_bark':
            ...
            x = layers.Conv2D(filters=8, kernel_size=(3,5), strides=(1,1), activation=None, padding='same')(encoder_input)
            x = layers.BatchNormalization()(x)
            x = layers.ReLU()(x)
            x = layers.Conv2D(filters=8, kernel_size=(3,5), strides=(1,1), activation=None, padding='same')(x)
            x = layers.BatchNormalization()(x)
            x = layers.ReLU()(x)
            ...
            dec = layers.BatchNormalization()(dec)
            dec = layers.ReLU()(dec)
            dec = layers.Conv2DTranspose(filters=1, kernel_size=(3,5), strides=2, padding='same', activation=None)(dec)
            dec = layers.BatchNormalization()(dec)
            dec = layers.ReLU()(dec)
            decoder_outputs = layers.Conv2DTranspose(filters=1, kernel_size=(3,5), strides=1, padding='same', activation="relu")(dec)
            ...
            checkpoint_path = "checkpoints_cvae_0_bark/cp_" + mode + "_" + str(it) + ".ckpt"
            ...
            with tf.device(gpu_name):
                ...
                history = model.fit(dataset_train, epochs=epochs, callbacks=[EarlyStoppingAtMinLoss(10,0),LearningRateSchedulerCustom(5),cp_callback], shuffle=True, validation_data=(dataset_test,None))  #  , callbacks=[early_stopping,lr_scheduler], shuffle=True, verbose=0
