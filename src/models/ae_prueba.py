...
for m in range(len(modes)):
    ...
    if mode=='unsupervised' or mode=='unsupervised_bark' or mode=='ae_bark':
    ...
    for it in range(1):
        ...
        if mode=='unsupervised' or mode=='unsupervised_bark' or mode=='ae_bark':
            ...
            x = layers.Conv2D(filters=8, kernel_size=9, strides=(1,1), activation=None, padding='same')(encoder_input)
            x = layers.BatchNormalization()(x)
            x = layers.ReLU()(x)
            x = layers.Conv2D(filters=8, kernel_size=9, strides=(1,1), activation=None, padding='same')(x)
            x = layers.BatchNormalization()(x)
            x = layers.ReLU()(x)
            x = layers.MaxPool2D(pool_size=(2, 2), padding='valid')(x)
            x = layers.Conv2D(filters=16, kernel_size=7, strides=(1,1), activation=None, padding='same')(x)
            x = layers.BatchNormalization()(x)
            x = layers.ReLU()(x)
            x = layers.Conv2D(filters=16, kernel_size=7, strides=(1,1), activation=None, padding='same')(x)
            x = layers.BatchNormalization()(x)
            x = layers.ReLU()(x)
            x = layers.MaxPool2D(pool_size=(2, 2), padding='valid')(x)
            x = layers.Conv2D(filters=32, kernel_size=5, strides=(1,1), activation=None, padding='same')(x)
            x = layers.BatchNormalization()(x)
            x = layers.ReLU()(x)
            x = layers.Conv2D(filters=32, kernel_size=5, strides=(1,1), activation=None, padding='same')(x)
            x = layers.BatchNormalization()(x)
            x = layers.ReLU()(x)
            x = layers.MaxPool2D(pool_size=(2, 2), padding='valid')(x)
            x = layers.Conv2D(filters=64, kernel_size=3, strides=(1,1), activation=None, padding='same')(x)
            x = layers.BatchNormalization()(x)
            x = layers.ReLU()(x)
            x = layers.Conv2D(filters=64, kernel_size=3, strides=(1,1), activation=None, padding='same')(x)
            x = layers.BatchNormalization()(x)
            x = layers.ReLU()(x)
            x = layers.MaxPool2D(pool_size=(2, 2), padding='valid')(x)
            x = layers.Conv2D(filters=128, kernel_size=3, strides=(1,1), activation=None, padding='same')(x)
            x = layers.BatchNormalization()(x)
            x = layers.ReLU()(x)
            x = layers.Conv2D(filters=128, kernel_size=3, strides=(1,1), activation=None, padding='same')(x)
            x = layers.BatchNormalization()(x)
            ...
            dec = layers.BatchNormalization()(dec)
            dec = layers.ReLU()(dec)
            dec = layers.Conv2DTranspose(filters=16, kernel_size=5, strides=2, padding='same', activation=None)(dec)
            dec = layers.BatchNormalization()(dec)
            dec = layers.ReLU()(dec)
            dec = layers.Conv2DTranspose(filters=16, kernel_size=5, strides=1, padding='same', activation=None)(dec)
            dec = layers.BatchNormalization()(dec)
            dec = layers.ReLU()(dec)
            dec = layers.Conv2DTranspose(filters=8, kernel_size=7, strides=2, padding='same', activation=None)(dec)
            dec = layers.BatchNormalization()(dec)
            dec = layers.ReLU()(dec)
            dec = layers.Conv2DTranspose(filters=8, kernel_size=7, strides=1, padding='same', activation=None)(dec)
            dec = layers.BatchNormalization()(dec)
            dec = layers.ReLU()(dec)
            dec = layers.Conv2DTranspose(filters=1, kernel_size=9, strides=2, padding='same', activation=None)(dec)
            dec = layers.BatchNormalization()(dec)
            dec = layers.ReLU()(dec)
            decoder_outputs = layers.Conv2DTranspose(filters=1, kernel_size=9, strides=1, padding='same', activation="relu")(dec)
            ...
            checkpoint_path = "checkpoints_ae_bark/cp_" + mode + "_" + str(it) + ".ckpt"
            ...
            with tf.device(gpu_name):
                ...
                history = model.fit(dataset_train, epochs=epochs, callbacks=[EarlyStoppingAtMinLoss(10,0.0),LearningRateSchedulerCustom(5),cp_callback], shuffle=True, validation_data=(dataset_test,None))  #  , callbacks=[early_stopping,lr_scheduler], shuffle=True, verbose=0
