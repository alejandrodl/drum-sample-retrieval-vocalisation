...
for m in range(len(modes)):
    ...
    if mode=='unsupervised' or mode=='unsupervised_bark' or mode=='ae_55':
    ...
    for it in range(3):
        ...
        if mode=='unsupervised' or mode=='unsupervised_bark' or mode=='ae_55':
            ...
            x = layers.ReLU()(x)
            x = layers.Conv2D(filters=8, kernel_size=(5,5), strides=(1,1), activation=None, padding='same')(x)
            x = layers.BatchNormalization()(x)
            x = layers.ReLU()(x)
            x = layers.Conv2D(filters=8, kernel_size=(5,5), strides=(1,1), activation=None, padding='same')(x)
            x = layers.BatchNormalization()(x)
            ...
            dec = layers.BatchNormalization()(dec)
            dec = layers.ReLU()(dec)
            dec = layers.Conv2DTranspose(filters=1, kernel_size=5, strides=1, padding='same', activation=None)(dec)
            dec = layers.BatchNormalization()(dec)
            dec = layers.ReLU()(dec)
            ...
            checkpoint_path = "checkpoints_ae_55/cp_" + mode + "_" + str(it) + ".ckpt"
